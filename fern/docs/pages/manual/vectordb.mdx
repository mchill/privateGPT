...
You can set the default collection name used to store the vectors by setting `vectorstore.collection_name`.
The collection can also be overridden on a per-request basis by setting the `collection_name` parameter in the request.
